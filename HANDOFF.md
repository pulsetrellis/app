...
- Public site: https://pulsetrellis.com/
...
The canonical origin is https://pulsetrellis.com/. The domain is registered through Squarespace and assigned to this Render site; `www` redirects to the apex domain. Keep `liveUrl` in `content/seo.json`, repository homepage, sitemaps and DNS aligned. The original https://pulsetrellis.onrender.com/ address remains available during migration. Verify HTTPS before publishing domain changes and update the backend origin restrictions before enabling connections on a new domain.
